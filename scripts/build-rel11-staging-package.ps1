...
$packageRoot = [IO.Path]::GetFullPath((Join-Path $temporaryRoot 'rel11-staging-package'))
$zipPath = [IO.Path]::GetFullPath((Join-Path $temporaryRoot 'aloitussivu-rel11-staging.zip'))
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $workspaceRoot 'docs/rel11-julkaisuportin-testimatriisi-2026-08-31.md') -Destination (Join-Path $packageRoot 'REL11_TEST_MATRIX.md') -Force
