...
$packageRoot = [IO.Path]::GetFullPath((Join-Path $temporaryRoot 'rel07-staging-package'))
$zipPath = [IO.Path]::GetFullPath((Join-Path $temporaryRoot 'aloitussivu-rel07-staging.zip'))
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $workspaceRoot 'deploy/cloudcity/README.md') -Destination (Join-Path $packageRoot 'DEPLOY_INSTRUCTIONS.md') -Force
