...
| Version 0.74.7 tuotantovienti | PASS 28.8.2026: `REL-11-v0.74.7-7bc8b9f06334`, paketin SHA-256 `dc135a6c27249fd9ed555a803cbc0a11f206c717a3c45ed1bc6eba5c86ac3d6d`, pääbundle `assets/main-Iuu9MbpP.js` ja julkinen health `ok/up/v1` |
| Version 0.74.7 tuotantovaikutusalue | Osittain PASS 28.8.2026: 320/375/768/1280 px × 100/200 %, 39 kategoriapainiketta, 320 px / 200 % -palauteikkuna, OP-linkit ja konsolivirheet 0 hyväksyttiin. Palautteen lähetyksen jälkeistä tuotantotilaa ei testattu uudella tietokantakirjoituksella. |
| WordPress-esittelysivun linkki | FAIL 28.8.2026: näkyvä `Avaa Seniorin aloitussivu` -painike on `<a>` ilman `href`-attribuuttia eikä osoita `/aloitus/`-palveluun |
| 1.0.0 / 1.9. tiedotuksen GO-aika | NO-GO 28.8.2026 klo 20.32; korjaa WordPressin P1-linkki, uusi palautteen tuotantopolku ja hanki riippumaton loppuhyväksyntä |
...
Version 0.74.7 TypeScript-tarkistus, salaisuustarkistus, Functions-build sekä Cloudcity-, staging-, Firebase-palautus- ja local-provider-buildit ovat PASS. Paikallinen PHP-ajonaika puuttui Windows-ympäristöstä, joten muuttumattoman API:n sopimustestejä ei ajettu uudelleen; viimeisin dokumentoitu 46/46-tulos säilyy vertailunäyttönä.

Tuotantopolun paketti rakennettiin puhtaasta commitista `7bc8b9f06334`. ZIPissä oli 115 tiedostoa, 68 julkista ja 45 yksityistä tiedostoa sekä 43 PHP-tiedostoa; vaarallisia polkuja ja kiellettyjä salaisuuksia oli 0. Paketin koko oli 826419 tavua ja SHA-256 `dc135a6c27249fd9ed555a803cbc0a11f206c717a3c45ed1bc6eba5c86ac3d6d`. Aktivoinnin jälkeen tuotannon etusivu ja uusi `assets/main-Iuu9MbpP.js` palauttivat 200, `/aloitus` ohjasi yhdellä 301-vastauksella kanoniseen `/aloitus/`-osoitteeseen, health palautti 200 sekä `ok/up/v1` ja `Cache-Control: no-store`, ja oma puuttuvan sivun näkymä palautti 404:n. Korjauksia sisältävät `FeedbackModal-pZKG8XEG.js` ja `constants-bY9coII6.js` latautuivat 200-vastauksella.

Tuotannon selainmatriisi läpäisi kaikki kahdeksan 320/375/768/1280 px × 100/200 % -yhdistelmää. Kaikki 39 kategoriapainiketta pysyivät näkymässä. Palauteikkunan kaikki ohjaimet pysyivät 320 px / 200 % -näkymässä, yleinen OP-linkki osoitti valtakunnalliselle henkilöasiakkaiden yhteystietosivulle ja `OP Keski-Suomi – seniorit ja erityistä tukea tarvitsevat` säilyi yksiselitteisesti alueellisena. Konsolivirheitä tai -varoituksia oli 0. Tuotantoon ei lähetetty uutta testipalautetta, joten korjatun onnistumistilan end-to-end-uusinta jää ennen 1.0.0-päätöstä tehtäväksi.

WordPressin WP-01–WP-04-jälkisavu läpäisi: etusivu, Etäopastus, Ajankohtaista, Yhteystiedot ja haku palauttivat 200, `/wp-admin/` ohjasi kirjautumiseen ja 152080 tavun JPEG-verrokkikuva säilyi. Erillinen WP-05-uusinta löysi kuitenkin P1-virheen: `https://seniorsurf.fi/aloitussivu/`-sivun näkyvä `Avaa Seniorin aloitussivu` -painike on selaimen DOMissa ankkuri ilman `href`-attribuuttia eikä saavutettavuuspuussa linkki. Löydös ei edellytä toimivan 0.74.7-sovelluspaketin palauttamista, mutta se estää version 1.0.0 ja laajan tiedotuksen GO-päätöksen, kunnes linkki on korjattu ja WordPress-jälkisavu toistettu.
